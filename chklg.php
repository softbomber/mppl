...
$sshUser = 'root'; // Логин (обычно везде root, если разный - добавьте 'user' в массив ниже)
...
$serversMap = [
    ...
    '45.90.217.114' => [
        'pass' => 'uikjm9', 
        'port' => 22  // Нестандартный порт
    ],
    '83.136.233.101' => [
        'pass' => 'bossismyname', 
        'port' => 45822 // Другой порт
    ],
    '103.213.249.5' => [
        'pass' => 'SSK4w6DfSGzk', 
        'port' => 45822 // Другой порт
...
// Получаем все устройства с нарушениями
...
// Получаем все заблокированные устройства с причиной anti-zapping
...
if (!empty($blockedKeys) && is_array($blockedKeys)) {
    foreach ($blockedKeys as $key) {
        ...
        if ($infoJson) {
            $info = json_decode($infoJson, true);
            // Извлекаем token и session_id из ключа
            if (preg_match('/blocked:devices:([^:]+):info:(.+)/', $key, $matches)) {
                ...
                // Показываем только anti-zapping блокировки
...
if ($_POST['action'] === 'check_input') {
    ...
    try {
        ...
        foreach ($allSessionsMap as $hash => $data) {
            ...
            // Если пользователь онлайн и смотрит канал, берем IP из аллокации
        ...
        // ... (ДАЛЕЕ КОД ОСТАЕТСЯ БЕЗ ИЗМЕНЕНИЙ ДО КОНЦА БЛОКА)
        $chMap = [];
        if (!empty($chIds)) {
        ...
        <div class="result-box">
             <!-- ... ВЕСЬ HTML ВЫВОДА ОСТАВЛЯЕМ КАК БЫЛ ... -->
                    ...
                    ?>
                        <div class="session-card" style="<?= $cardStyle ?>">
                            ...
                            <div class="card-body">
                                ...
                                <!-- ВОТ ЗДЕСЬ ТЕПЕРЬ БУДЕТ ПОКАЗЫВАТЬСЯ CDN -->
...
// 9. ПОЛУЧЕНИЕ СПИСКА ВСЕХ ОНЛАЙН ПОЛЬЗОВАТЕЛЕЙ (ИСПРАВЛЕНО)
if ($_POST['action'] === 'get_online_users') {
    try {
        $redis = getRedis($redisConfig);
        
        // 1. Получаем список имен каналов из БД
        $chMap = [];
        try {
        ...
        // 2. Сканируем Redis (Используем сырую команду SCAN через execute)
        $usersFound = [];
        $now = time();
        
        // Начальный курсор должен быть строкой '0'
        $cursor = '0';
        
        do {
            // Вызываем SCAN: курсор, MATCH паттерн, COUNT (порциями по 1000)
            ...
            // Redis возвращает массив: [0] => следующий_курсор, [1] => список_ключей
            $cursor = $response[0];
            $keys = $response[1];
            
            if (!empty($keys) && is_array($keys)) {
                foreach ($keys as $key) {
                    // Пропускаем ключи метаданных (:meta)
                    if (strpos($key, ':meta') !== false) continue;
                    
                    // $key выглядит как "online:users:TOKEN123"
                    // Отрезаем префикс (13 символов)
                    $token = substr($key, 13);
                    if ($token) {
        ...
        // Прекращаем, когда курсор вернулся в '0'
        } while ($cursor != '0');
...
$tokenShort = mb_strlen($row['token']) > 15 ? mb_substr($row['token'], 0, 15)."..." : $row['token'];
            ...
            echo "<tr>";
//            echo "<td><a href='#' onclick='switchToUser(\"{$row['token']}\"); return false;' style='color:#6c96bb; text-decoration:none; border-bottom:1px dashed #444'>{$tokenShort}</a></td>";
...
if ($_POST['action'] === 'get_streams') {
    try {
        ...
        $channelIds = array_unique(array_column($streams, 'channel_id'));
        $names = [];
        if (!empty($channelIds)) {
        ...
        if (empty($streams)) {
            ...
            foreach ($streams as $s) {
                ...
                $url = $s['source_url'] ?? '';
	        $token = $s['token'] ?? '';
                $chName = htmlspecialchars($names[$cid] ?? "ID $cid");

		$rowsHtml .= "<tr id='stream-row-$cid' data-token='" . htmlspecialchars($token, ENT_QUOTES) . "'>";
                $rowsHtml .= "<td>$chName</td>";               // ← название канала (новый столбец!)
                $rowsHtml .= "<td class='num'>$cid</td>";
                ...
                $rowsHtml .= "<button class='btn-mini-action check' onclick='checkStream(this, \"$cid\", \"$cdn\")' title='Проверить процесс (SSH)'>⚡</button> ";
		$tokenEsc = htmlspecialchars($token, ENT_QUOTES);
                $rowsHtml .= "<button class='btn-mini-action delete' onclick='deleteStream(\"$cid\", \"$slot\", \"$prov\",\"$tokenEsc\")' title='Остановить (Publish Stop)'>✕</button>";
                $rowsHtml .= "</td>";
                $rowsHtml .= "</tr>";
            }
        }
...
// 8. ПРОВЕРКА ПРОЦЕССА (SSH: PS AUX)
   if ($_POST['action'] === 'check_stream_alive') {
        ...
        // Получаем настройки для текущего IP
        ...
        // Извлекаем данные (если порт не задан, берем 22)
        $currentPass = $conf['pass'] ?? '';
        $currentPort = $conf['port'] ?? 22; 
        
        // ИСПРАВЛЕННАЯ КОМАНДА:
        // 1. ps auxww - чтобы строка не обрезалась
        // 2. grep -w '{$cid}' - ищем точное совпадение числа (без слешей)
        $cmd = "ps auxww | grep hls_p | grep -w '{$cid}' | grep -v grep";
        $output = '';
        ...
        if (function_exists('ssh2_connect')) {
            ...
            // ВАРИАНТ 2: shell_exec + sshpass
            // Экранируем пароль
            $safePass = escapeshellarg($currentPass);
            
            // Важно: sshpass должен быть установлен на сервере (apt install sshpass)
...
html, body {
    width: 100%;
    overflow-x: hidden; /* Ключевое: запрещает скролл всей страницы */
}
...
table { 
    ...
    min-width: 500px; /* Таблица будет скроллиться, если экран меньше 500px */
}
.top-bar { background: black; padding: 10px 12px;  display: flex; justify-content: space-between; position: sticky; top: 0; z-index: 1000; width:100%; max-width:100vw}
   ...
   /* .container { width: 100%; max-width: 900px; margin: 30px auto; padding: 0 15px; overflow-x: hidden;}*/
.container { 
    width: 100%; 
    max-width: 900px; 
    margin: 30px auto; 
    padding: 0 15px; 
    display: block; /* Сброс flex, чтобы дети не растягивали контейнер */
}
    ...
    /* Search */
    /*.search-wrap { position: relative; background: #121d27; padding:2px; border-radius: 30px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); display: flex; align-items: center; border: 1px solid #1c3551 }*/
...
.search-wrap { 
    ...
    /* ИСПРАВЛЕНИЕ: */
    width: fit-content;        /* Ширина по контенту, а не 100% */
    max-width: 500px;          /* Но не шире 500px */
    margin: 0 auto 20px auto;  /* Теперь это сработает */
}
    ...
    <div class="stats-card">
        <table id="streamsTable">
            ...
            <tbody id="streamsBody">
                <tr><td colspan="7" style="text-align:center; padding:20px; color:#999">Загрузка...</td></tr>
            </tbody>
        </table>
    </div>
</div>
...
function checkStream(btn, cid, cdnIp) {
const row = document.getElementById('stream-row-' + cid);
btn.innerHTML = '...';
btn.disabled = true;
...
} else if (status.trim() === 'DEAD') {
row.classList.add('row-dead');
btn.style.color = '#dc3545';
//alert('Процесс hls_p не найден на сервере!');
} else {
...
// 1. Обновляем switchView, чтобы он понимал новую вкладку
// (Замените старую функцию switchView или дополните её условие)
/* 
   ВНИМАНИЕ: Найдите существующую функцию switchView и добавьте туда 
   блок else if (viewName === 'users') { ... } 
   Либо замените её целиком на эту версию:
*/
function switchView(viewName) {
    document.querySelectorAll('.view-section').forEach(el => el.classList.remove('active'));
    document.querySelectorAll('.nav-btn').forEach(el => el.classList.remove('active'));
    ...
    const btns = document.querySelectorAll('.nav-btn');
    // Порядок кнопок важен! 0=Search, 1=Streams, 2=Stats, 3=Users, 4=Zapping
    if (viewName === 'search') btns[0].classList.add('active');
    else if (viewName === 'streams') {
        btns[1].classList.add('active');
        loadStreams();
...
// Быстрый переход к поиску конкретного токена при клике
...
function loadZappingDevices() {
    // Загружаем устройства с нарушениями
    ...
    // Получаем устройства с нарушениями
    ...
    // Получаем заблокированные устройства
...
function resetZappingViolations(token, sessionId) {
    ...
    const fd = new FormData();
    fd.append('action', 'unblock_zapping_device');
    fd.append('token', token); // <--- ТЕПЕРЬ ПЕРЕДАЕМ ТОКЕН
    fd.append('session_id', sessionId);
    
    fetch('', { method: 'POST', body: fd }) // <-- Убрал 'chklg.php', используем текущий URL
        .then(r => r.text())
        .then(resp => {
            if (resp.trim() === 'OK') {
                loadZappingDevices();
...
function sortUsers(n, type) {
    // Упрощенная сортировка для вкладки Users (можно расширить как в sortStreams)
    ...
    rows.sort((a, b) => {
        ...
        // Для колонки Duration (3) преобразуем "01:00:00" в секунды для правильной сортировки
        if(n === 3) {
             x = parseDuration(x);
             y = parseDuration(y);
             return dir === 'asc' ? x - y : y - x;
