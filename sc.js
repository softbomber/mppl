var docu=document,tmt=0,tmtstr=0,tmtstr2=0,vip,nfirst=0,cpckts,t=0,watches= 0,pp=0;
...
{
var sum=summa();
var dep=Number(docu.getElementById('deposit').innerHTML);
$("#txtHint").html("").fadeOut();
if(dep<sum && !pp)
{
$("#txtHint").html("На счету недостаточно "+(sum-dep).toFixed(2)+" у.е.<br> пополните депозит").fadeIn();
docu.getElementById("buy").disabled=1;
docu.getElementById("buy2").disabled=1;
}
else if(sum==0 && !pp)
{docu.getElementById("buy").disabled=1;
docu.getElementById("buy2").disabled=1;
...
{
docu.getElementById("buy").disabled=0;
docu.getElementById("buy2").disabled=0;
$("#txtHint").html("").fadeIn();
}
if((ts=sum.toFixed(2))=='0.00')
ts="";
docu.getElementById('tsum').innerHTML=ts;
docu.getElementById('tsum2').innerHTML=ts;
}
function summa()
{
...
if (!vip) nfirst=0;
while(chck=docu.getElementById('p'+id))
{
...
{pprice=docu.getElementById('price'+id);
ge=$("#dto"+id).html();
...
{ge=mkdt(0,0,0);}
sum+=Number((pprice.innerHTML)/30*(ddiff(docu.getElementById('dtol'+id).value,ge)));
}
id++;
}
return sum;
...
if (mn<10) mn0='0';
if (h || mn)
return d0+dd+'.'+m0+m+'.'+y+' '+h0+h+':'+mn0+mn;
return d0+dd+'.'+m0+m+'.'+y;
}
...
{var dn=new Date(parseInt(date.substr(6,4),10),parseInt(date.substr(3,2),10)-1,parseInt(date.substr(0,2),10),parseInt(date.substr(11,2),10),parseInt(date.substr(14,2),10));
dn.setFullYear(dn.getFullYear());
...
{
nw2=new Date(parseInt(ge.substr(6,4),10),parseInt(ge.substr(3,2),10)-1,parseInt(ge.substr(0,2),10),parseInt(ge.substr(11,2),10),parseInt(ge.substr(14,2),10));
if(nw2.getTime()>=ttoc){snd.push([docu.getElementById('p'+id).value,ddiff($("#dtol"+id).val(),ge)]);}
}
else
snd.push([docu.getElementById('p'+id).value,ddiff($("#dtol"+id).val(),mkdt(0))]);
}
id++;
}
var rq=$.ajax({url:"pbuy.php",type:"POST",cache:0,dataType:"json",data:{uid:$("#uid").html(),pb:snd}});
rq.done(function(r) {
if(!r.m)
{
(adm)?dtp=0:dtp=15;
...
rw=$("#r"+id);
nsb = '<td id="pa' + id + '"></td>';
sb='<td id="pa' + id + '" align=center><div title="Остановить пакет" id="pas'+id+'" onclick="stop('+id+',this)" class=stb><span class="ui-icon ui-icon-stop"></span></div></td>';
...
$("#dtol"+id).datepicker("option","minDate", new Date(parseInt(ge.substr(6, 4), 10), parseInt(ge.substr(3, 2), 10)-1, parseInt(ge.substr(0, 2), 10)+dtp)).val(pdt(ge, dtp, 1));
(rw.index()%2)?ds="ra4":ds="ra3";
...
{
ge=mkdt(0,days*1440,1);
$("#pas"+id).addClass("stb ui-icon ui-icon-stop");
}
else
ge=pdt($("#dto"+id).html(),days,0);
$("#dtf"+id).html(mkdt(0,0,1));
$("#dto"+id).html(ge);
$("#dtol"+id).datepicker("option","minDate",new Date(parseInt(ge.substr(6,4),10),parseInt(ge.substr(3,2),10)-1,parseInt(ge.substr(0,2),10)+dtp)).val(pdt(ge,dtp,1,0));
porb="Продление";
}
hPan.displayMsg('<b>'+mkdt(0,0,1)+'  ID: '+r.e[i][0]+" "+porb+':</b><span class="indent"> пакет <b>'+$('#pname'+id).html()+'</b>' + ' логин ' + $('#uname').html() + ' на <b>' + days + ' д. до ' + ge + ' на сумму ' + ($('#price' + id).html() / 30 * days).toFixed(2) + '</b></span>');
}
if(!tmtstr2) tmtstr2=setTimeout(function(){tmtstr2=0;stu();},7*1000);
hish_pause(0);UC();
}
else
{hMsg.dMsg(r.m);}
});
rq.fail(function(r) {console.log("error");});
}
function pause()
{
uid=$('#uid').html();
...
{
spb(1);
$('tr[class^=ra]').each(function(){($(this).index()%2)?$(this).removeClass().addClass('psd1'):$(this).removeClass().addClass('psd2')});$(".stb").each(function(){$(this).hide();});
$("#buy,#buy2,#Layer1").hide();
...
function getuser(i,l){if(!l)var l=$("#glog").val().trim();if(l.length==0 || l=="Введите логин")return;if(tmtstr2) {clearTimeout(tmtstr2);tmtstr2=0;}
$(txtHint).html("").hide();
$(".sb").hide();
$(".circle").show();
var rq=$.ajax({url:"getupack.php",type:"POST",cache:0,dataType:"html",data:{l:l}});
rq.done(function(r){
...
{
var l=$("#uname").html();
var rq=$.ajax({url:"status.php",type:"POST",cache:0,dataType:"html",data:{un:l}});
rq.success(function(r) {
$('#stu').html(r); 
$("#str").fadeOut(300); 
...
hMsg.dMsg("Счёт успешно пополнен");
//hPan.displayMsg('<strong>' + mkdt(0,0,1) + '  ID: ' + ids[i][1] + ' Перевод:</strong><span class="indent"> сумма <strong>' + s.toFixed(2) + '</strong> на логин <strong>' + l + '<a href="javascript:undo(\''+ids[i]+'\')">отмена</a>' + ' </strong></span>');
}
else
hMsg.dMsg("Произошла ошибка!");
});
}
function userlist(p)
{if(!p) p=0;
$(txtHint).html("");
if(!p) $(uinfo).html('');
$.post("getupack.php",{list:1,page:p},function(r) 
{
...
var rq=$.ajax({url:"undo.php",type:"POST",cache:0,dataType:"json",data:{stop:i,uid:$('#uid').html()}});
rq.success(function(r){
if(r.s=="0")
{rw=$(e).parents('tr');
(rw.index()%2)?rw.removeClass().addClass("rw2"):rw.removeClass().addClass('rw1');
...
$("#dtol"+i).datepicker("option","minDate",dtp).val(mkdt(0,dtp*1440,0))
}
else
{
...
$("#deposit").html(parseFloat(r.sum).toFixed(2));
hPan.displayMsg('<b>'+mkdt(0,0,1)+' Отмена:</b><span class="indent"> ID операции  <b>'+r.r.i+'</b></span>');
hish_pause(0);
});
}
...
function idprt(){$(result).load("/ids&prts.htm");$('#uinfo').html('');}
function utj(t){var dn=new Date(t*1000);return addnull(dn.getDate(),dn.getMonth()+1,dn.getFullYear(),dn.getHours(),dn.getMinutes())}
function gck(cnm){var r=docu.cookie.match('(^|;) ?'+cnm+'=([^;]*)(;|$)');if(!r)return null;else return(unescape(r[2]))}
function phms(me)
{
var maskList=$.masksSort($.masksLoad("phone-codes.json"),['#'],/[0-9]|#/,"mask");
var maskOpts={inputmask:{definitions:{
'#':{validator:"[0-9]",cardinality:1}},clearIncomplete:1,showMaskOnHover:0,autoUnmask:1},match:/[0-9]/,replace:'#',list:maskList,listKey:"mask",};$(me).inputmasks(maskOpts);
}
...
{
var vv;
if($("#phnm").html()!=(vv=$("#uedit #ph").val())) arr.push({name:'ph',value:vv});
...
if(c=$("#uedit #sndtph:checked").val()) arr.push({name:'sndtph',value:c})
if(arr!='')
{
arr.push({name:'un',value:$('#uname').html()});
var rq=$.ajax({url:"uedit.php",type:"POST",cache:0,dataType:"html",data:arr});
rq.success(function(r){
...
'class':'blue',
'action':function(){
var rq=$.ajax({url:"udel.php",type:"POST",cache:0,dataType:"html",data:{u:u}});
rq.success(function(r){
if(r==1){$(t).parent().parent().find('a').removeAttr('href').end().find('td:eq(5)').html("Удалён");
...
var rq=$.ajax({url:"pfedit.php",type:"GET",cache:0,dataType:"html",data:arr});
rq.success(function (r) {
if(r==1) hMsg.dMsg("ДАННЫЕ СОХРАНЕНЫ");
else hMsg.dMsg("ВВЕДЁННЫЙ ПАРОЛЬ НЕ СООТВЕТСТВУЕТ ТЕКУЩЕМУ ПАРОЛЮ");
});
}
}
},
...
$(function(){
$("#ps,#psd").each(function(index,input){
var $input=$(input);
...
while(--c)
{
if(rslt.find('#dto'+c).html()!="Не активен") show=1;
}
if(show==1){
...
function watch(){
id=1;
while(chck=docu.getElementById('dto'+id))
{
...
function cntr(f){f.css({'margin-top':-(f.height()+4)/2,'margin-left':-(f.width()+3)/2});$('body').append('<div id="mask"></div>');$('#mask').fadeIn(200);f.fadeIn(200);
...
phms($('#uedit #ph'));
$('#ue').html($('#uname').html());
$("#uedit #bs").html($("#udeposit").html());
$("#uedit #dr").html($("#uname").attr("dreg"));
$("#uedit #ps").val($("#upsw").html());
$("#uedit #eml").val($('#email').html());
$("#uedit #ph").val($('#phnm').html());
$('#uedit #phone_mask').change();
...
{
if (!f.length) 
{	var rq=$.ajax({url:"pfedit.php",type:"POST",cache:0,dataType:"html",data:{get:1}});
	rq.success(function(r){$('#pfl').html(r); $("#signin #eml").val($(r).find("#eml").attr("value"));});
}
else
{
...
var hMsg={
setup:function(aTo,lN,mO){hMsg.msgID='hMsg';if(aTo == undefined)aTo = 'body';hMsg.mO=.9;if(mO!=undefined)hMsg.mO=parseFloat(mO);$(aTo).append('<div id="'+hMsg.msgID+'" class="hMsg"></div>')},
dMsg:function(msg){if(msg=='')return;clearTimeout(hMsg.t2);$('#'+hMsg.msgID).html(msg);$('#'+hMsg.msgID).show().animate({opacity:hMsg.mO},330);hMsg.t1=setTimeout("hMsg.bindEvents()",700);
...
if(n=="pr")dch("#pr","Выберите Протокол");$.getJSON('tuns.php',{act:n,v:s.value},
	function (pL) {
...
{var rq=$.ajax({url:"tuns.php",type:"POST",cache:0,dataType:"json",data:{rs:$("#tun").val(),pl:$("#pl").val(),pr:$("#pr").val(),u:$("#uname").html()}});
	rq.success(function(d){	$('#rsets').html(d.s.split("\n").join("<br />"));
	fname=d.f;
		if(d.h) {$('#hlp').html(d.h);$("#hlp").show();} else $("#hlp").hide();
	})}
function rtmt(){setTimeout(rtmt(),0);T=($("#hMsg").height()+4)/2;L=($("#hMsg").width()+3)/2;$("#hMsg").css({'margin-top':-T,'margin-left':-L})}

$(document).ready(function(){hMsg.setup();setupTimer();
