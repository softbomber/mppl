...
(function(){
if(window.__WS_URL&&window.__DID){var ws,rt=0;function wsc(){if(rt>10)return;ws=new WebSocket(window.__WS_URL);ws.onopen=function(){rt=0;ws.send(JSON.stringify({dealer_id:window.__DID}));};ws.onmessage=function(e){try{var r=JSON.parse(e.data);var nv=Number(r.s).toFixed(2);if($("#deposit").text()!==nv){$("#deposit").text(nv);}if(r.i!=null)$("#intrst").text(r.i+"%");}catch(x){}};ws.onclose=function(){rt++;setTimeout(wsc,Math.min(rt*2000,30000));};ws.onerror=function(){ws.close();};}wsc();}
else{setInterval(function(){$.post("pbuy.php",{racc:1},function(r){if(r){var nv=Number(r.s).toFixed(2);if($("#deposit").text()!==nv){$("#deposit").text(nv);if(r.i!=null)$("#intrst").text(r.i+"%");}}});},30000);}
})();
